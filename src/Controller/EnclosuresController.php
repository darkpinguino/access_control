    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Enclosures->save($enclosure)) {
                $this->Flash->success(__('The enclosure has been saved.'));
                return $this->redirect(['action' => 'index']);
            } else {
                $this->Flash->error(__('The enclosure could not be saved. Please, try again.'));
            }
        }
        $companies = $this->Enclosures->Companies->find('list', ['limit' => 200]);
        $this->set(compact('enclosure', 'companies'));
        $this->set('_serialize', ['enclosure']);
    }
